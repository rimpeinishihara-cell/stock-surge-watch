...
6. Discordに投稿する
7. 状態(ミュート設定・最終処理メッセージID)を `state/` にコミットして保存
...
```
!help                        コマンド一覧
!mute 2330                   銘柄をミュート(今後一切通知しない、無期限)
!mute 2330 6203 8995 30日    複数銘柄を期間限定でミュート(30日/4週/1ヶ月などを指定可)
!unmute 2330                 ミュート解除(複数指定可: !unmute 2330 6203)
!muted                       現在のミュート設定を表示
```

`!mute` は期間を省略すると無期限、末尾に `30日`/`4週`/`1ヶ月` のような期間を付けると、
その日数が経過した時点で(次回の実行時に)自動的にミュート解除される。
...
ミュートされた銘柄は、掲示板取得・Claude呼び出しともスキップされるため無駄なコストがかからない。
